...
def parse_anima_lbws_anima(lbws_anima):
    if lbws_anima is None:
        return None

    expected_keys = set(ANIMA_LBW_SUBLAYERS)
    parsed = {}
    for item in lbws_anima.split(","):
        if not item or "=" not in item:
            raise ValueError("--lbws_anima must use self_attn=<number>,cross_attn=<number>,mlp=<number> format.")
        key, value = item.split("=", 1)
        key = key.strip()
        value = value.strip()
        if key not in expected_keys:
            raise ValueError(f"Unknown --lbws_anima key '{key}'. Expected: self_attn, cross_attn, mlp.")
        if key in parsed:
            raise ValueError(f"Duplicate --lbws_anima key '{key}'.")
        try:
            parsed[key] = float(value)
        except ValueError as err:
            raise ValueError(f"--lbws_anima value for '{key}' must be a number.") from err

    missing = expected_keys - set(parsed)
    if missing:
        raise ValueError(f"--lbws_anima is missing required keys: {', '.join(sorted(missing))}.")
    return parsed


def parse_anima_lbws(lbws, model_count, require_arrays=False):
    if not lbws:
        return None
    if len(lbws) != model_count:
        raise ValueError("Number of --lbws entries must be equal to number of models.")

    try:
        parsed_lbws = [json.loads(lbw) for lbw in lbws]
    except json.JSONDecodeError as err:
            raise ValueError("Each --lbws entry must be a JSON array or object.") from err

    normalized_lbws = []
    for lbw in parsed_lbws:
        if isinstance(lbw, list):
            if len(lbw) != ANIMA_LBW_BLOCK_COUNT:
                raise ValueError(f"Each --lbws array must contain exactly {ANIMA_LBW_BLOCK_COUNT} values for Anima blocks 0-27.")
            if not all(isinstance(weight, (int, float)) for weight in lbw):
                raise ValueError("All --lbws values must be numbers.")
            normalized_lbws.append({sublayer: lbw for sublayer in ANIMA_LBW_SUBLAYERS})
            continue

        if require_arrays:
            raise ValueError("--lbws_anima can only be used with 28-value JSON array entries in --lbws.")
        if not isinstance(lbw, dict):
            raise ValueError("Each --lbws entry must be a 28-value JSON array or an object with self_attn, cross_attn, and mlp arrays.")
        missing = [sublayer for sublayer in ANIMA_LBW_SUBLAYERS if sublayer not in lbw]
        if missing:
            raise ValueError(f"Structured --lbws is missing required keys: {', '.join(missing)}.")
        unknown = set(lbw) - set(ANIMA_LBW_SUBLAYERS)
        if unknown:
            raise ValueError(f"Structured --lbws contains unknown keys: {', '.join(sorted(unknown))}.")
        normalized = {}
        for sublayer in ANIMA_LBW_SUBLAYERS:
            weights = lbw[sublayer]
        ...
        normalized_lbws.append(normalized)

    return normalized_lbws


def get_anima_lbw(base_module_name, lbw, lbws_anima=None):
    if lbw is None:
        return 1.0
    ...
    if block_index >= ANIMA_LBW_BLOCK_COUNT:
    ...
    if suffix.startswith("self_attn_") or suffix.startswith("adaln_modulation_self_attn_"):
        sublayer = "self_attn"
    elif suffix.startswith("cross_attn_") or suffix.startswith("adaln_modulation_cross_attn_"):
        sublayer = "cross_attn"
    elif suffix.startswith("mlp_") or suffix.startswith("adaln_modulation_mlp_"):
        sublayer = "mlp"
    else:
        return 1.0
    multiplier = lbws_anima[sublayer] if lbws_anima is not None else 1.0
    return lbw[sublayer][block_index] * multiplier


def format_anima_lbw_summary(model_path, ratio, lbw):
    lines = []
    for sublayer in ANIMA_LBW_SUBLAYERS:
        ...
        lines.append(f"Anima strengths for {model_path} ({sublayer}): [{formatted_strengths}]")
    return lines


def format_anima_lbws_anima_summary(lbws_anima):
    return " / ".join(
        f"{sublayer}={format_anima_strength(lbws_anima[sublayer])}"
        for sublayer in ANIMA_LBW_SUBLAYERS
    )
...
def merge_lora_models(model_paths, ratios, lbws, lbws_anima, new_rank, new_conv_rank, device, merge_dtype,
                      no_clamp, quantize, quantize_bits, quantize_method, quantize_seed,
                      filter_by_first_lora, module_filter_rules_parsed,
                      svd_mode, svd_oversample, svd_niter, show_lbws=False):
    ...
    for i, (model_path, ratio) in enumerate(zip(model_paths, ratios)):
        ...
        if show_lbws:
            for summary in format_anima_lbw_summary(model_path, ratio, lbws[i]):
                logger.info(summary)
    ...
    if lbws_anima is not None:
        logger.info(f"Anima shared sublayer strengths: {format_anima_lbws_anima_summary(lbws_anima)}")
    ...
    with torch.no_grad():
        for base_lora_module_name in tqdm(modules_to_process, desc="Processing modules"):
            ...
            for i, (model_path, ratio) in enumerate(zip(model_paths, ratios)):
                ...
                lbw = lbws[i] if lbws is not None else None
                ...
                accumulated_effect += ratio * lora_effect * module_config["strength"] * get_anima_lbw(base_lora_module_name, lbw, lbws_anima)
...
def merge(args):
    ...
    lbws_anima = parse_anima_lbws_anima(args.lbws_anima)
    lbws_input = args.lbws
    if lbws_anima is not None and not lbws_input:
        default_lbw = json.dumps([1.0] * ANIMA_LBW_BLOCK_COUNT)
        lbws_input = [default_lbw] * len(args.models)
    lbws = parse_anima_lbws(lbws_input, len(args.models), require_arrays=lbws_anima is not None)
    ...
    state_dict, metadata = merge_lora_models(
        args.models, args.ratios, lbws, lbws_anima, args.new_rank,
        ...
        args.svd_mode, args.svd_oversample, args.svd_niter, show_lbws=bool(args.lbws)
    )
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--lbws", type=str, nargs="*", default=[],
        help="Anima block weights as one 28-value JSON array per model. Values map to lora_unet_blocks_0 through lora_unet_blocks_27.",
    )
    parser.add_argument(
        "--lbws_anima", type=str, default=None,
        help="Shared Anima sublayer multipliers. Requires --lbws arrays: self_attn=<number>,cross_attn=<number>,mlp=<number>.",
    )
    parser.add_argument(
